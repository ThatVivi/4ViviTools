...
Sequence:
  [1] extract icons  -> icons/raw/<class>/0.png            (skills from iconpack.zip, + GRF monster sprites)
...
"""
import argparse, glob, hashlib, json, os, random, shutil, sys, zipfile
...
MONSTER_MANIFEST = os.path.join(ICONS, "monster_manifest.json")

def log(m): print(m, flush=True)

def monster_manifest_labels():
    try:
        data = json.load(open(MONSTER_MANIFEST, encoding="utf-8"))
        labels = {c.get("label", "") for c in data.get("classes", [])}
        return {x for x in labels if x.startswith("mob__")}
    except Exception:
        return set()

def selected_raw_classes():
    all_classes = sorted(os.path.basename(d) for d in glob.glob(os.path.join(RAW, "*")) if os.path.isdir(d))
    manifest_mobs = monster_manifest_labels()
    manifest_mobs_lower = {m.lower() for m in manifest_mobs}
    allowed_prefixes = ("skills__", "skill__")
    selected = []
    skipped = []
    for c in all_classes:
        if c.startswith("mob__"):
            if not manifest_mobs or c.lower() in manifest_mobs_lower:
                selected.append(c)
            else:
                skipped.append(c)
            continue
        if c.startswith("map__") or c.startswith("spr_"):
            skipped.append(c)
            continue
        if c.startswith(allowed_prefixes):
            ...
            skipped.append(c)
    if skipped:
        log("[1] skipped %d non-target raw classes (items/maps/player sprites/old mobs/etc.)" % len(skipped))
    if manifest_mobs:
        log("[1] using monster manifest: %d current mob__ classes" % len(manifest_mobs))
    return selected
    ...
    for c in classes:
        ...
        for p in sorted(glob.glob(os.path.join(RAW, c, "*.png"))):
            try:
                ...
                pass
    return {"count": total, "sha1": h.hexdigest()}

def augment(classes, variants, img, raw_sig=None, val_frac=0.1):
    ...
    json.dump({"classes": len(classes), "variants": variants, "img": img, "raw": raw_sig}, open(AUGMETA, "w"))
...
def main():
    ...
    extract_icons()
    raw_classes = selected_raw_classes()
    raw_sig = raw_signature(raw_classes)
    ...
    if a.skip_data or data_ready:
        ...
        augment(raw_classes, a.variants, a.img, raw_sig=raw_sig)
