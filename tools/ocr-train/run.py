...
def env_imports_ok(use_gpu):
    ...
    if r.returncode != 0:
        ...
        return False
    if use_gpu and "cuda_compiled True" not in (r.stdout or ""):
        log("    dependency check found CPU Paddle while GPU training was requested")
        return False
    return True
...
def main():
    ...
    if os.path.exists(stamp) and env_imports_ok(use_gpu):
        ...
        ensure_env(use_gpu)
        if not env_imports_ok(use_gpu):
            raise RuntimeError("Paddle/PaddleOCR dependencies still fail after install.")
