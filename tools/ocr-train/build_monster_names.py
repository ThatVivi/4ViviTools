...
def extra_frames(spr_path, k=6):
    ...
    recognizer matches it from any direction instead of only the single idle pose."""
    idx_frames, rgba_frames = decode_spr(spr_path)
...
def append_to_bank(new_classes):
    ...
    for lbl, paths in new_classes.items():
        ...
        for p in paths[have:target]:
            todo.append((lbl, p))
    if not todo:
        log("[append] every monster already has %d references -> nothing to add." % target); return
...
def main():
    ...
    ap.add_argument("--src", default=DEFAULT_SRC, help="folder with monster .spr/.act pairs")
    ...
    for sp in sprs:
        ...
        label = "mob__" + "".join(ch if ch.isalnum() or ch == "_" else "_" for ch in name)
        ...
        try:
            extras = extra_frames(sp, k=TARGET_REFS - 1)
        except Exception:
            extras = []
        ei = 1
        for ex in extras:
    ...
    log("rendered %d monsters, %d total frames (%d failed) -> icons/raw/mob__* (up to %d poses each)"
        % (len(grouped), total_frames, fail, TARGET_REFS))

    if not a.no_append:
        append_to_bank(grouped)
        sync_runtime_copies()
        regen_vision_pack()
    log("DONE.")
