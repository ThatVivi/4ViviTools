...
public sealed partial class CalculatorViewModel : ViewModelBase
{
    ...
    {
        // totals from base + manual enchants + every equipped item's bonuses
        int G(EquipInfo e, string k) => e.Bonuses.TryGetValue(k, out var v) ? v : 0;
        int sum(string k) => Build.Sum(e => G(e, k));

        Damage.BaseLevel = BaseLevel;
        Damage.Str = Str + EnchStr + sum("Str");
        Damage.Agi = Agi + EnchAgi + sum("Agi");
        Damage.Vit = Vit + EnchVit + sum("Vit");
        Damage.Intt = Intel + EnchInt + sum("Int");
        Damage.Dex = Dex + EnchDex + sum("Dex");
        Damage.Luk = Luk + EnchLuk + sum("Luk");
        Damage.WeaponDamage = WeaponAtk;

        Damage.Gear.Clear();
        int flatAtk = sum("Atk");
        if (flatAtk != 0)
            Damage.Gear.Add(new GearBonusRow { Name = "Build gear (+ATK)", FlatAtk = flatAtk });

        Damage.CalculateCommand.Execute(null);
    }
