...
public sealed class LootRow { public string Time { get; set; } = ""; public string Item { get; set; } = ""; public int Qty { get; set; } = 1; public long Value { get; set; } = 0; }
...
public sealed class LootLog
{
    ...
    public long TotalValue => Rows.Sum(r => r.Value * r.Qty);
    public int TotalCount => Rows.Sum(r => r.Qty);
    public void Add(string item, int qty) => Add(item, qty, 0);
    public void Add(string item, int qty, long valueEach) { Rows.Insert(0, new LootRow { Time = DateTime.Now.ToString("HH:mm:ss"), Item = item, Qty = qty, Value = valueEach }); Save(); }
